...
static void action_set_region(region_data* data, char* name, bool populated) {
    ...
    if (R_SUCCEEDED(set_region_for_title((info->titleId), region_from_string(name)))) {
        // Refresh locale info
        ...
        error_display(data, NULL, "Failed to set region\n(does locales directory exist?)");
    }
}
...
void action_change_region(linked_list* items, list_item* selected) {
    ...
    if (info->mediaType == MEDIATYPE_NAND){
        error_display(NULL, NULL, "Cannot set region for system titles.");
        return;
    }
    ...
    data->populated = true;
    data->count = RGN_MAX;
    list_display_old("Select Region", "A: Select, B: Return", data, region_update, region_draw_top);
}
